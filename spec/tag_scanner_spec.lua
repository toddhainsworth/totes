...
describe("tag_scanner.notes_for_tag", function()
  ...
  it("returns files whose tags match an exact prefix", function()
    write_note(dir, "a.md", "---\ntags:\n  - project/totes\n---\n")
    write_note(dir, "b.md", "---\ntags:\n  - area/health\n---\n")
    local results = scanner.notes_for_tag(dir, "project/totes")
    assert.equals(1, #results)
    assert.is_true(results[1]:match("a%.md$") ~= nil)
  end)

  it("returns files whose tags fall under a hierarchical prefix", function()
    write_note(dir, "a.md", "---\ntags:\n  - project/totes\n---\n")
    write_note(dir, "b.md", "---\ntags:\n  - project/other\n---\n")
    write_note(dir, "c.md", "---\ntags:\n  - area/health\n---\n")
    local results = scanner.notes_for_tag(dir, "project/")
    assert.equals(2, #results)
  end)

  it("returns all files when prefix is empty", function()
    write_note(dir, "a.md", "---\ntags:\n  - project/totes\n---\n")
    write_note(dir, "b.md", "---\ntags:\n  - area/health\n---\n")
    local results = scanner.notes_for_tag(dir, "")
    assert.equals(2, #results)
  end)

  it("returns empty table when no files match the prefix", function()
    write_note(dir, "a.md", "---\ntags:\n  - area/health\n---\n")
    local results = scanner.notes_for_tag(dir, "project/")
    assert.same({}, results)
  end)

  it("does not double-count a file with multiple matching tags", function()
    write_note(dir, "a.md", "---\ntags:\n  - project/totes\n  - project/other\n---\n")
    local results = scanner.notes_for_tag(dir, "project/")
    assert.equals(1, #results)
  end)
